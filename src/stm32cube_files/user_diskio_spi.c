...
#define FCLK_FAST() { MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_8); }	/* Set SCLK = fast, (was approx 4.5 MBits/s at prescale 8. Now running at 18) */
...
{
	BYTE rxDat;
    HAL_StatusTypeDef err = HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 100);
    return rxDat;
}
...
{
    HAL_StatusTypeDef err = HAL_SPI_Transmit(&SD_SPI_HANDLE, (uint8_t*)buff, btx, 500);
  //HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, (uint8_t*)buff, 0, btx, 500);
...
)
{
	...
	rcvr_spi_multi(buff, btr);		/* Store trailing data to the buffer */ // @CC
